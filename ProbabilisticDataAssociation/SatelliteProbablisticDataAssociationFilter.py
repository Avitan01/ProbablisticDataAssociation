...
class SatelliteProbabilisticDataAssociationFilter:
    ...
    def validate(self, measurements: set) -> set:
        ...
        self._gamma = (np.sqrt(np.linalg.det(self._S)) / len(measurements)) * (
            stats.norm.ppf(1 - (1 - self._Pg) / self.NUMMEAS))
    ...
    @property
    def state_large(self) -> np.array:
        return_array = np.zeros(len(self._x))
        return_array[0] = 1000*self._x[0]
        return_array[1:] = self._x[1:]
